...
func tagHostHeuristic(h *Host) {
	...
	plcProtocolCount := 0
	if hasModbus { plcProtocolCount++ }
	if hasS7 { plcProtocolCount++ }
	if hasOmron { plcProtocolCount++ }
	if hasMitsubishi { plcProtocolCount++ }
	if hasIO { plcProtocolCount++ }
	if initENIP > 0 || rcvENIP > 0 { plcProtocolCount++ }
