...
func (p *PCAPParser) Parse() (*interfaces.NetworkModel, error) {
	// Open PCAP file
	...
	model := &interfaces.NetworkModel{
		...
		Policies: []*interfaces.SecurityPolicy{}, // Empty for PCAP-only
...
func (p *PCAPParser) GetMetadata() interfaces.InputMetadata {
...
func (p *PCAPParser) GetType() interfaces.InputType {
	return interfaces.InputTypePCAP
}

// processPacket processes a single packet and updates the model
func (p *PCAPParser) processPacket(packet gopacket.Packet, model *interfaces.NetworkModel) error {
	...
	flowKey := interfaces.FlowKey{
		SrcIP: srcAsset.ID,
		DstIP: dstAsset.ID,
		Proto: interfaces.Protocol(protocol),
	}
	...
	if flow == nil {
		flow = &interfaces.Flow{
			...
			Protocol:    interfaces.Protocol(protocol),
...
func (p *PCAPParser) processL2Protocol(packet gopacket.Packet, model *interfaces.NetworkModel, eth *layers.Ethernet) error {
	...
	if model.Flows[flowKey] == nil {
		model.Flows[flowKey] = &interfaces.Flow{
			Source:      srcAsset.ID,
			Destination: dstAsset.ID,
			Protocol:    interfaces.Protocol(protocol),
			FirstSeen:   packet.Metadata().Timestamp,
	...
	srcAsset.Protocols = p.addProtocolIfNotExists(srcAsset.Protocols, interfaces.Protocol(protocol))
	dstAsset.Protocols = p.addProtocolIfNotExists(dstAsset.Protocols, interfaces.Protocol(protocol))

	return nil
}

// getOrCreateAsset retrieves or creates an asset
func (p *PCAPParser) getOrCreateAsset(model *interfaces.NetworkModel, ip, mac string) *interfaces.Asset {
	...
	if asset == nil {
		asset = &interfaces.Asset{
			...
			PurdueLevel:  interfaces.Unknown,
			IEC62443Zone: "", // Will be inferred later
			Criticality:  interfaces.LowAsset,
...
func (p *PCAPParser) updateAssetProtocols(srcAsset, dstAsset *interfaces.Asset, protocol string, tcpLayer, udpLayer gopacket.Layer) {
	proto := interfaces.Protocol(protocol)
...
func (p *PCAPParser) addProtocolIfNotExists(protocols []interfaces.Protocol, proto interfaces.Protocol) []interfaces.Protocol {
...
func (p *PCAPParser) enhanceModel(model *interfaces.NetworkModel) error {
...
func (p *PCAPParser) inferNetworkSegments(model *interfaces.NetworkModel) {
	// Group assets by network patterns (simplified)
	networks := make(map[string][]*interfaces.Asset)
...
func (p *PCAPParser) classifyAssetPurdueLevel(asset *interfaces.Asset, model *interfaces.NetworkModel) interfaces.PurdueLevel {
	...
	for _, proto := range asset.Protocols {
		...
		case "EtherNet/IP I/O", "Profinet":
			return interfaces.L1 // Field devices
		case "EtherNet/IP", "Modbus TCP", "S7Comm":
			// Check if more client or server behavior
			return interfaces.L2 // Supervisory control
		case "OPC-UA":
			return interfaces.L2 // Could be L2 or L3
		}
	}
	return interfaces.Unknown
}

func (p *PCAPParser) classifyAssetZone(asset *interfaces.Asset, model *interfaces.NetworkModel) interfaces.IEC62443Zone {
...
func (p *PCAPParser) assessAssetCriticality(asset *interfaces.Asset, model *interfaces.NetworkModel) interfaces.CriticalityLevel {
	// Simple heuristic based on protocols and level
	if asset.PurdueLevel == interfaces.L1 {
		return interfaces.HighAsset // Field devices are critical
	}
	return interfaces.MediumAsset
}

func (p *PCAPParser) assessAssetExposure(asset *interfaces.Asset, model *interfaces.NetworkModel) interfaces.ExposureLevel {
	// For PCAP analysis, assume OT-only unless proven otherwise
	return interfaces.OTOnly
}

func (p *PCAPParser) inferDeviceName(asset *interfaces.Asset) string {
...
func (p *PCAPParser) inferNetworkZone(assets []*interfaces.Asset) interfaces.IEC62443Zone {
	// Majority vote from assets
	zoneCount := make(map[interfaces.IEC62443Zone]int)
	for _, asset := range assets {
		zoneCount[asset.IEC62443Zone]++
	}

	maxCount := 0
	var dominantZone interfaces.IEC62443Zone
...
func (p *PCAPParser) assessNetworkRisk(assets []*interfaces.Asset) interfaces.RiskLevel {
	highRiskCount := 0
	for _, asset := range assets {
		if asset.Criticality == interfaces.HighAsset || asset.Criticality == interfaces.CriticalAsset {
			highRiskCount++
		}
	}

	if highRiskCount > len(assets)/2 {
		return interfaces.HighRisk
...
func (p *PCAPParser) inferNetworkPurpose(assets []*interfaces.Asset) string {
